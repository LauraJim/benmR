...
3. Postulating a simple shape for the niche of a species (Hutchinson's fundamental niche) and using the confidence curves form a multivariate normal distribution to represent the fundamental niche, so that the parameters of interest are a vector of means, $\mu$, and a precision matrix, $A$ (which is defined as the inverse of the covariance matrix).  
...
5. Applying an MCMC algorithm (the one coded in the package *Rtwalk*) to simulate from the posterior and analyzing the variation in the sample.

### *benmR*: package description

The *benmR* package contains all the functions that the users need to apply this model to their data (*priorpar*,*Supp*, *Initth*,*Energy*,*Runtw*), as well as to produce visualizations of their data (*plotdata*) and the results of the niche estimation (*plot_iter*,*uncer_plot*). It also contains to datasets, *backAM* and *Spocc*, that can be used to run the examples provided here and in the documentation of each function.  
